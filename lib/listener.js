...
class Listener {
  ...
  hookListeners(trackingOptions, callback) {
    trackingOptions.trackers.forEach(tracker => {
      ...
      } else if (tracker.type === TRACKING_TYPE.CLICK_CAPTURE) {
        trackingOptions.element.addEventListener('click', callback.bind(null, tracker.event, tracker.type, trackingOptions.element), {capture: true});
  ...
  registerIntersectionObserver(tracker, element, callback) {
    new IntersectionObserver(this.onIntersection.bind(null, tracker, element, callback)).observe(element);
  }
